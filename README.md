...
A 1.2m wingspan solar-electric FPV glider — prototyping toward extended
...
deployed independently (e.g. to Vercel). It's a read-only viewer over a
bundled snapshot of this repo's markdown files (refreshed manually via its
`scripts/sync-content.sh`, not a live GitHub API read) — a nicer front-end
than hand-editing files, for viewing only. All edits still happen here,
through Claude Code. It does not run the Python scripts in `calculations/`
— those stay Claude Code's job. See that repo's own README for setup.
